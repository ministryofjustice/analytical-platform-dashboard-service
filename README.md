...
## Install with uv
WIP very early stages, so not much to see. However, you to install the project locally with `uv`:

1. Install `uv` ([I recommend using homebrew](https://docs.astral.sh/uv/getting-started/installation/#homebrew))
1. Then simply run `uv sync` from the root of the project. This will install the correct python version, create a `venv` and install dependencies.
1. You can then run commands with `uv run` e.g. `uv run python manage.py runserver`. Alternatively, source your venv as normal and run that way. See the `uv` docs for further guidance.

## Database for local development
You will need postgres installed locally. Assuming you are using a Mac, you can run `createdb dashboard_service` to create the database. Then you can migrate the database with `uv run python manage.py migrate`


## Original template instructions TODO
